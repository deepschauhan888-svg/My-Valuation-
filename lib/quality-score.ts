...
// not a hidden model. Category-backed facts (unit type, furnishing, age,
// legal issues) are read from `attributes` by the same keys the seeded rule
// categories use — if an admin renames or removes a category, the check
// degrades gracefully (attribute simply won't be present) rather than erroring.
const CHECKS: {
  label: string;
  weight: number;
  ...
  { label: "Similar configuration (unit type)", weight: 20, test: (s, c) => s.attributes.unitType === c.attributes.unitType },
  {
    label: "Construction age within 5 years",
    weight: 15,
    test: (s, c) => Math.abs((parseFloat(s.attributes.age) || 0) - (parseFloat(c.attributes.age) || 0)) <= 5,
  },
  {
    label: "Comparable area within 15%",
    weight: 15,
    test: (s, c) => {
  ...
  { label: "No flagged legal issues", weight: 10, test: (_s, c) => c.attributes.legalIssues !== "true" },
  { label: "Similar furnishing level", weight: 5, test: (s, c) => s.attributes.furnishing === c.attributes.furnishing },
];
...
export function scoreComparable(subject: PropertyInput, comparable: PropertyInput): ComparableQualityScore {
  ...
  const percent = Math.round(reasons.reduce((sum, r, i) => sum + (r.met ? CHECKS[i].weight : 0), 0));
